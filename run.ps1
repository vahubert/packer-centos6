...
if( Test-Path -Path $build_directory_hyperv1 ){
   Write-Host -ForegroundColor green -BackgroundColor black Deleting $build_directory_vmware
   Remove-Item -Recurse -Force $build_directory_hyperv1
}
...
if (!$builder) {
packer build -var-file="variables.json" -on-error=ask centos6.json
}
else {
packer build -var-file="variables.json" -only="$builder" -on-error=ask centos6.json
}
